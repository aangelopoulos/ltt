# import some common libraries
import numpy as np
import torch
import matplotlib.pyplot as plt
import os, json, cv2, random, sys, traceback
from utils import *

import multiprocessing as mp

import pickle as pkl
from tqdm import tqdm
import pdb


def get_loss_tables():
    print("Getting the loss tables!")
    with torch.no_grad():
        manager = mp.Manager()
        global_dict = manager.dict({"loss_tables": None, "boxes": None, "roi_masks": None, "softmax_outputs": None, "gt_classes": None, "gt_masks": None, "curr_proc": 0})
	# Load cache
        with open('./.cache/boxes.pkl', 'rb') as f:
            global_dict["boxes"] = pkl.load(f)[:15]

        with open('./.cache/roi_masks.pkl', 'rb') as f:
            global_dict["roi_masks"] = pkl.load(f)[:15]

        with open('./.cache/softmax.pkl', 'rb') as f:
            global_dict["softmax_outputs"] = pkl.load(f)[:15]

        with open('./.cache/gt_classes.pkl', 'rb') as f:
            global_dict["gt_classes"] = pkl.load(f)[:15]

        with open('./.cache/gt_masks.pkl', 'rb') as f:
            global_dict["gt_masks"] = pkl.load(f)[:15]

        lambda1s = torch.linspace(0,1,5) # Top score threshold
        lambda2s = torch.linspace(0,1,5) # Segmentation threshold
        lambda3s = torch.linspace(0.8,1,10) # APS threshold

        num_processes = 15 
        global_dict['loss_tables'] = torch.zeros(3,lambda1s.shape[0],lambda2s.shape[0],lambda3s.shape[0])

        iou_correct = 0.5

        # Execute loss calculations in parallel
        pbar = tqdm(total=lambda1s.shape[0]*lambda2s.shape[0]*lambda3s.shape[0])
        jobs = []

        for i in range(lambda1s.shape[0]):
            for j in range(lambda2s.shape[0]):
                for k in range(lambda3s.shape[0]):
                    confidence_threshold = lambda1s[i] 
                    segmentation_threshold = lambda2s[j] 
                    aps_threshold = lambda3s[k] 
                    p = mp.Process(target=eval_detector_wrapped, args=(i,j,k, global_dict, confidence_threshold, segmentation_threshold, aps_threshold, iou_correct))
                    while global_dict['curr_proc'] >= num_processes:
                        time.sleep(2)
                    jobs.append(p)
                    p.start()
                    global_dict['curr_proc'] += 1
                    pbar.update(1)

        pbar.close()

        for proc in jobs:
            proc.join()

        torch.save(loss_tables, './.cache/loss_tables.pt')

# Evaluates the detector and stores the losses in the loss table
def eval_detector_wrapped(i,j,k, global_dict, confidence_threshold, segmentation_threshold, aps_threshold, iou_correct):
    print(f"Started ({i},{j},{k})")
    neg_m_coverage, neg_miou, neg_recall = eval_detector(global_dict, confidence_threshold, segmentation_threshold, aps_threshold, iou_correct)
    global_dict['loss_tables'][0,i,j,k] = neg_m_coverage
    global_dict['loss_tables'][1,i,j,k] = neg_miou
    global_dict['loss_tables'][2,i,j,k] = neg_recall
    print(f"({i},{j},{k}) | nmc: {neg_m_coverage:.3f}, nmiou: {neg_miou:.3f}, nrec: {neg_recall:.3f}")
    global_dict['curr_proc'] -= 1

# Three risks: coverage (APS), 1-mIOU@50, and 1-recall
def eval_detector(global_dict, confidence_threshold, segmentation_threshold, aps_threshold, iou_correct):
    running_corrects = 0
    running_total = 0
    running_gt = 0
    running_sum_mean_covered_perimage = 0
    running_sum_miou_perimage = 0
    for i in range(len(global_dict["roi_masks"])):
        corrects, ious, unused, covered = eval_image(i,global_dict,confidence_threshold,segmentation_threshold,aps_threshold,iou_correct)
        if corrects == None:
            continue
        running_corrects += ((corrects + (ious > iou_correct)).float() >= 2).float().sum()
        running_total += float(corrects.shape[0])
        running_gt += float(global_dict["gt_masks"][i].shape[0])
        running_sum_miou_perimage += float(ious.mean())
        running_sum_mean_covered_perimage += np.nan_to_num(float(covered[ious >= iou_correct].mean()))
        neg_m_coverage = 1 - running_sum_mean_covered_perimage / float(i+1) 
        neg_miou = 1 - running_sum_miou_perimage/float(i+1)
        neg_recall = running_corrects/running_gt

    return neg_m_coverage, neg_miou, neg_recall 

# Calculates per-image metrics 
def eval_image(i, global_dict, confidence_threshold, segmentation_threshold, aps_threshold, iou_correct):
    pred_masks = global_dict["roi_masks"][i].to_bitmasks(global_dict["boxes"][i],global_dict["gt_masks"][i].shape[1],global_dict["gt_masks"][i].shape[2],segmentation_threshold).tensor

    if global_dict["softmax_outputs"][i].shape[0] == 0:
       return None, None, None, None 

    top_scores, indices = global_dict["softmax_outputs"][i].max(dim=1)[0].sort(descending=True)
    filter_idx = top_scores > confidence_threshold 

    softmax_output = global_dict["softmax_outputs"][i][filter_idx]
    box = global_dict["boxes"][i][filter_idx]
    roi_mask = global_dict["roi_masks"][i][filter_idx]
    # must reindex
    if softmax_output.shape[0] == 0:
       return None, None, None, None 

    top_scores, indices = softmax_output.max(dim=1)[0].sort(descending=True)

    est_classes = softmax_output.argmax(dim=1)

    # Setup for APS
    test_sorted, test_pi = softmax_output.sort(dim=1, descending=True)
    _, class_ranks = test_pi.sort(dim=1, descending=False)
    sizes = (test_sorted.cumsum(dim=1) <= aps_threshold).int().sum(dim=1)
    sizes = torch.max(sizes,torch.ones_like(sizes))
    #rank_of_true = (pi == labels_ind[:index_split,None]).int().argmax(dim=1) + 1

    # Starting with the most confident mask, correspond it with a ground truth mask based on IOU
    ious = torch.zeros_like(top_scores)
    corrects = torch.zeros_like(top_scores)
    covered = torch.zeros_like(top_scores)
    unused = torch.tensor(range(global_dict["gt_masks"][i].shape[0]))
    for index in indices:
        if unused.shape[0] == 0:
            break
        _int = (pred_masks[index] * global_dict["gt_masks"][i][unused]).sum(dim=1).sum(dim=1) 
        _uni = ((pred_masks[index].int() + global_dict["gt_masks"][i][unused].int()) >= 1).sum(dim=1).sum(dim=1)
        _iou = _int.float()/_uni.float()
        ious[index], max_iou_idx = (_iou.max().item(), _iou.argmax().item())
        curr_gt_class = global_dict["gt_classes"][i][unused][max_iou_idx]
        corrects[index] = curr_gt_class == est_classes[index]
        
        # Deal with APS
        covered[index] = class_ranks[index,curr_gt_class] <= (sizes[index] - 1)

        unused = unused[unused != unused[max_iou_idx]]

    return corrects, ious, unused, covered

if __name__ == "__main__":
    fix_randomness(seed=0)
    mp.set_start_method('fork')
    get_loss_tables()
